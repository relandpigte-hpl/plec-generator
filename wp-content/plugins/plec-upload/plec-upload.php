<?php
/**
 * Plugin Name: PLEC Upload
 * Description: HTML file upload manager powered by React + Vite.
 * Version: 1.0.0
 * Author: Reland Pigte
 */

if (!defined('ABSPATH')) {
    exit;
}

final class Plec_Upload_Plugin {
    private const HANDLE = 'plec-upload-app';
    private const SHORTCODE = 'plec_upload';
    private const DEV_SERVER_DEFAULT = 'http://localhost:5174';
    private const DEV_SERVER_DEBUG = true;
    private const UPLOAD_SUBDIR = 'plec-uploads';
    private const ALLOWED_MIME_TYPES = [
        'html' => 'text/html',
        'htm'  => 'text/html',
    ];
    private const MODULE_HANDLES = [
        'plec-upload-app',
        'plec-upload-app-vite-client',
    ];

    public function __construct() {
        add_shortcode(self::SHORTCODE, [$this, 'render_shortcode']);
        add_filter('script_loader_tag', [$this, 'filter_script_loader_tag'], 10, 3);
        add_action('wp_ajax_plec_upload_files', [$this, 'handle_upload']);
    }

    public function render_shortcode(): string {
        if (!$this->can_current_user_use()) {
            return '';
        }

        $this->enqueue_assets();

        return '<div id="plec-upload-root"></div>';
    }

    private function enqueue_assets(): void {
        if ($this->enqueue_dev_assets()) {
            return;
        }

        $this->enqueue_build_assets();
    }

    private function enqueue_dev_assets(): bool {
        if (!self::DEV_SERVER_DEBUG) {
            return false;
        }

        $dev_server = self::DEV_SERVER_DEFAULT;

        $response = wp_remote_get($dev_server . '/@vite/client', [
            'timeout'   => 0.3,
            'sslverify' => false,
        ]);

        if (is_wp_error($response) || wp_remote_retrieve_response_code($response) !== 200) {
            return false;
        }

        wp_enqueue_script(self::HANDLE . '-vite-client', $dev_server . '/@vite/client', [], null, true);
        wp_script_add_data(self::HANDLE . '-vite-client', 'type', 'module');

        wp_enqueue_script(self::HANDLE, $dev_server . '/src/main.jsx', [], null, true);
        wp_script_add_data(self::HANDLE, 'type', 'module');
        $this->localize_config();

        return true;
    }

    private function enqueue_build_assets(): void {
        $manifest_path = plugin_dir_path(__FILE__) . 'dist/.vite/manifest.json';

        if (!file_exists($manifest_path)) {
            return;
        }

        $manifest = json_decode((string) file_get_contents($manifest_path), true);

        if (!is_array($manifest) || !isset($manifest['src/main.jsx'])) {
            return;
        }

        $entry = $manifest['src/main.jsx'];

        if (!empty($entry['file'])) {
            wp_enqueue_script(
                self::HANDLE,
                plugin_dir_url(__FILE__) . 'dist/' . $entry['file'],
                [],
                null,
                true
            );
            wp_script_add_data(self::HANDLE, 'type', 'module');
            $this->localize_config();
        }

        if (!empty($entry['css']) && is_array($entry['css'])) {
            foreach ($entry['css'] as $index => $css_file) {
                wp_enqueue_style(
                    self::HANDLE . '-style-' . $index,
                    plugin_dir_url(__FILE__) . 'dist/' . $css_file,
                    [],
                    null
                );
            }
        }
    }

    private function localize_config(): void {
        wp_localize_script(self::HANDLE, 'plecUploadConfig', [
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce'   => wp_create_nonce('plec_upload_files'),
            'siteUrl' => home_url(),
        ]);
    }

    public function filter_script_loader_tag(string $tag, string $handle, string $src): string {
        $is_handle = in_array($handle, self::MODULE_HANDLES, true);
        $is_vite   = strpos($src, '/@vite/client') !== false || strpos($src, '/src/main.jsx') !== false;

        if (!$is_handle && !$is_vite) {
            return $tag;
        }

        if (strpos($tag, 'type="module"') !== false || strpos($tag, "type='module'") !== false) {
            return $tag;
        }

        $tag_without_type = (string) preg_replace('/\stype=([\"\']).*?\\1/', '', $tag);

        if (strpos($tag_without_type, '<script') === 0) {
            return (string) preg_replace('/<script\b/', '<script type="module"', $tag_without_type, 1);
        }

        return sprintf('<script type="module" src="%s"></script>', esc_url($src));
    }

    public function handle_upload(): void {
        if (!$this->can_current_user_use()) {
            wp_send_json_error(['message' => 'Unauthorized.'], 403);
        }

        $nonce = isset($_POST['nonce']) ? sanitize_text_field((string) wp_unslash($_POST['nonce'])) : '';
        if (!wp_verify_nonce($nonce, 'plec_upload_files')) {
            wp_send_json_error(['message' => 'Invalid nonce. Please refresh the page.'], 403);
        }

        $rows = json_decode((string) wp_unslash($_POST['rows'] ?? ''), true);
        if (!is_array($rows) || empty($rows)) {
            wp_send_json_error(['message' => 'No files to upload.'], 400);
        }

        // Ensure upload directory exists.
        $upload_dir  = wp_upload_dir();
        $target_dir  = trailingslashit($upload_dir['basedir']) . self::UPLOAD_SUBDIR;

        if (!file_exists($target_dir)) {
            wp_mkdir_p($target_dir);
        }

        $uploaded = [];

        foreach ($rows as $row) {
            if (empty($row['field'])) {
                continue;
            }

            $field = $row['field'];

            if (!isset($_FILES[$field]) || !is_uploaded_file($_FILES[$field]['tmp_name'])) {
                continue;
            }

            $file     = $_FILES[$field];
            $original = sanitize_file_name($file['name']);

            // Validate HTML file type.
            $check = wp_check_filetype_and_ext($file['tmp_name'], $original, self::ALLOWED_MIME_TYPES);
            if (empty($check['ext']) || empty($check['type'])) {
                continue;
            }

            // Build unique filename: iterationName_uniqueid.html
            $iteration_name = !empty($row['iterationName'])
                ? sanitize_file_name($row['iterationName'])
                : pathinfo($original, PATHINFO_FILENAME);

            $unique_id = substr(uniqid('', true), -8);
            $safe_name = $iteration_name . '_' . $unique_id . '.html';

            // Prevent any collision (extremely unlikely but safe).
            $dest = trailingslashit($target_dir) . $safe_name;
            $counter = 0;
            while (file_exists($dest)) {
                $counter++;
                $safe_name = $iteration_name . '_' . $unique_id . '_' . $counter . '.html';
                $dest      = trailingslashit($target_dir) . $safe_name;
            }

            if (!move_uploaded_file($file['tmp_name'], $dest)) {
                continue;
            }

            $uploaded[] = [
                'original' => $original,
                'saved'    => $safe_name,
                'url'      => trailingslashit($upload_dir['baseurl']) . self::UPLOAD_SUBDIR . '/' . $safe_name,
            ];
        }

        if (empty($uploaded)) {
            wp_send_json_error(['message' => 'No valid HTML files were uploaded.'], 400);
        }

        wp_send_json_success([
            'message' => sprintf('Successfully uploaded %d file(s).', count($uploaded)),
            'files'   => $uploaded,
            'count'   => count($uploaded),
        ]);
    }

    private function can_current_user_use(): bool {
        if (!is_user_logged_in()) {
            return false;
        }

        $user = wp_get_current_user();
        if (!$user instanceof WP_User) {
            return false;
        }

        return in_array('administrator', $user->roles, true)
            || in_array('subscriber', $user->roles, true);
    }
}

new Plec_Upload_Plugin();
